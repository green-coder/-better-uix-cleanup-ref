...
(defn- cleanup-ref [ref]
  (when-let [^ratom/Reaction temp-ref (aget ref "__rat")]
    (ratom/add-on-dispose! temp-ref #(prn "temp-ref was disposed."))
    (prn "-alive before" @ratom/-reactions-alive)
    (prn "-running before" @ratom/-running)
    (ratom/dispose! temp-ref)
    (prn "-running after" @ratom/-running)
    (prn "-alive after" @ratom/-reactions-alive)
    ,))
...
(defn use-track [f & args]
  (use-reaction (ratom/make-track f args)))
